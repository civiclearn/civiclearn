/**
 * CivicAuth — Auth Guard (Sproochentest)
 * ═══════════════════════════════════════════════════════════════
 ...
 *   <script src="/sproochentest/assets/js/sproochentest-auth.js"></script>
 *
 * After loading, the global `sproochentestAuth` object is available:
 *   sproochentestAuth.email      — the logged-in user's email
 *   sproochentestAuth.userId     — the Supabase Auth user ID (UUID)
 *   sproochentestAuth.session    — the full session object
 *   sproochentestAuth.supabase   — the initialized Supabase client
 *   sproochentestAuth.signOut()  — logs out and redirects to login
 *   sproochentestAuth.ready      — Promise that resolves when auth check is complete
 */
...
(function () {
  ...
  const LOGIN_URL         = '/sproochentest/login.html';
  ...
  auth.ready = (async () => {
    try {
      ...
      window.dispatchEvent(new CustomEvent('sproochentestAuthReady', { detail: auth }));
