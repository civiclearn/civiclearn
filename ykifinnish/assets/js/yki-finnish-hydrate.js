...
(function () {
  ...
  var ENDPOINT = "https://htgliokekeaovdiafrgs.supabase.co/functions/v1/yki-finnish-hydrate";
  ...
  function hydrate() {
    ...
    if (sessionStorage.getItem("yki_finnish_hydrated")) {
      console.log("[YKI Finnish Hydrate] Already hydrated this session, skipping.");
      return;
    }

    var email = (localStorage.getItem("cl_email") || "").toLowerCase().trim();
    if (!email) {
      console.log("[YKI Finnish Hydrate] No email found, skipping.");
      return;
    }

    console.log("[YKI Finnish Hydrate] Fetching cloud data for:", email);
    ...
    fetch(ENDPOINT, {
      ...
      .then(function (result) {
        ...
        if (!attempts.length) {
          console.log("[YKI Finnish Hydrate] No remote data found.");
          sessionStorage.setItem("yki_finnish_hydrated", "1");
          return;
        }
        ...
        console.log("[YKI Finnish Hydrate] Restored", hydrated, "test(s) from cloud.");
        sessionStorage.setItem("yki_finnish_hydrated", "1");

        // Reload once so dashboard picks up the data
        if (hydrated > 0 && !sessionStorage.getItem("yki_finnish_hydrate_reloaded")) {
          sessionStorage.setItem("yki_finnish_hydrate_reloaded", "1");
          location.reload();
        }
      })
      .catch(function (err) {
        console.warn("[YKI Finnish Hydrate] Failed (non-blocking):", err.message);
      });
  }
