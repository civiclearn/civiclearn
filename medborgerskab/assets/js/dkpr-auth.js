/**
 * CivicAuth — Auth Guard (Denmark-PR)
 * ═══════════════════════════════════════════════════════════════
 * Include on every protected page.
 *
 * After loading, the global `denmarkprAuth` object is available:
 *   denmarkprAuth.email      — the logged-in user's email
 *   denmarkprAuth.userId     — the Supabase Auth user ID (UUID)
 *   denmarkprAuth.session    — the full session object
 *   denmarkprAuth.supabase   — the initialized Supabase client
 *   denmarkprAuth.signOut()  — logs out and redirects to login
 *   denmarkprAuth.ready      — Promise that resolves when auth check is complete
 */
...
(function () {
  ...
  auth.ready = (async () => {
    try {
      ...
      window.dispatchEvent(new CustomEvent('denmarkprAuthReady', { detail: auth }));
