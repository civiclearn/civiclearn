/**
 * CivicAuth — Auth Guard (YKI Svenska)
 * ═══════════════════════════════════════════════════════════════
 ...
 *   <script src="/ykisvenska/assets/js/ykisvenska-auth.js"></script>
 *
 * After loading, the global `ykisvenskaAuth` object is available:
 *   ykisvenskaAuth.email      — the logged-in user's email
 *   ykisvenskaAuth.userId     — the Supabase Auth user ID (UUID)
 *   ykisvenskaAuth.session    — the full session object
 *   ykisvenskaAuth.supabase   — the initialized Supabase client
 *   ykisvenskaAuth.signOut()  — logs out and redirects to login
 *   ykisvenskaAuth.ready      — Promise that resolves when auth check is complete
 */
...
(function () {
  ...
  const LOGIN_URL         = '/ykisvenska/login.html';
  ...
  auth.ready = (async () => {
    try {
      ...
      window.dispatchEvent(new CustomEvent('ykisvenskaAuthReady', { detail: auth }));
