/* ─────────────────────────────────────────────────────────
   CivicLearn Dashboard Extra — Cyprus
   Mastery ring + rows (v3) + Gauge + Celebration + Study time
...
(function () {
  ...
  function getTopicLabel(topic) {
    if (!topic) return "Misc";
    if (typeof topic === "object") return topic[lang] || topic.el || topic.en || "Misc";
    return topic;
  }
  ...
  function computeAndCacheGauge() {
    ...
    const passScore = simCfg.passScore || 15;
    ...
    // Badge label: shared dictionary first (gauge_badge_*), then a built-in el/ru/en fallback
    let badgeText;
    if (passed) {
      badgeText = t("gauge_badge_pass", lang === "el" ? "Θα περνούσατε" : lang === "ru" ? "Проходной балл" : "Would pass");
    } else if (close) {
      badgeText = t("gauge_badge_close", lang === "el" ? "Σχεδόν" : lang === "ru" ? "Почти" : "Almost there");
    } else {
      badgeText = t("gauge_badge_fail", lang === "el" ? "Χρειάζεται εξάσκηση" : lang === "ru" ? "Нужно больше практики" : "Needs practice");
    }
  ...
  // card has vertical presence next to the practice widget.
  ...
  // DOM list instead of a canvas. Long lists (>8 rows) get a "many" class and flows into two columns on desktop.
  function renderMasteryRows(cd) {
    const host = document.getElementById("masteryRows");
    if (!host) return;
    host.classList.toggle("many", cd.labels.length > 8);
    host.innerHTML = cd.labels.map(function (label, i) {
      const v = cd.data[i];
      const c = masteryColor(v);
      return '<div class="mastery-row">' +
               '<span class="mastery-row-label" title="' + String(label).replace(/"/g, "&quot;") + '">' + label + '</span>' +
  ...
  function renderChart() {
    ...
    // v3: colour by mastery level, not by topic index
      ...
      options: {
        ...
        plugins: {
          ...
          tooltip: {
            callbacks: {
              label: function (ctx) {
                return t("chart_tooltip_pct_mastered", "{n}% mastered").replace("{n}", ctx.parsed.y);
              }
            }
          }
        }
      ...
      plugins: [{
        ...
        afterDatasetsDraw(chart) {
          ...
          meta.data.forEach((bar, i) => {
            ...
            ctx.fillStyle = masteryColor(value);
