/* ────────────────────────────────────────────────────────
   Dashboard Sequential Practice — Denmark (v2)
   Uses main questions.json. Sequential order.
   Writes to civicedge_progress (same keys as engine-dk)
...
(function () {
  ...
  async function initSequential() {
    ...
    const COUNTRY = (window.CIVICEDGE_CONFIG && window.CIVICEDGE_CONFIG.country) || "denmark";
    ...
    function render() {
      ...
      // Detect format: DK flat = [{t, correct}], DKPR multilingual = ["string", ...]
      const hasCorrectIndex = typeof q.correctIndex === "number";

      const optionObjects = rawOpts.map((opt, i) => {
        // Object format: {t: "text", correct: true/false}
        if (typeof opt === "object" && opt !== null && !Array.isArray(opt)) {
          return {
            text: String(opt.t || opt.text || ""),
            correct: hasCorrectIndex ? (i === q.correctIndex) : !!opt.correct,
            originalIndex: i
          };
        }
        // String format (DKPR multilingual)
        return {
          text: String(opt),
          correct: hasCorrectIndex ? (i === q.correctIndex) : false,
          originalIndex: i
        };
      });
      ...
      shuffled.forEach((opt) => {
        ...
        btn.addEventListener("click", () => {
          ...
          // Write to civicedge_progress — same format as engine
    ...
    function recordToProgress(q, correct) {
      try {
        ...
        // DK engine key format: topicLabel:text
        // q.topic may be string (flat) or object (multilingual)
        const topicLabel = (typeof q.topic === "object")
          ? (q.topic.da || q.topic.en || "topic")
          : (q.topic || "topic");
        const qText = (typeof q.q === "object")
          ? (q.q.da || q.q.en || "")
          : (q.q || "");
        const key = `${topicLabel}:${qText}`;
        ...
        // Always refresh _raw snapshot (needed by dashboard-extra-dk)
        entry._raw = {
        ...
        // Also mark as answered for unseen-preference sampling
    ...
    function recordToStats(q, correct, userAnswerText, correctAnswerText) {
      try {
        ...
        // Find or create today's sequential session
