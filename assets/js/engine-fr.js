...
(function () {
  ...
  feedback: (q && typeof q.feedback === "string") ? q.feedback : null
};
...
Engine.start = async function start(mode, options = {}) {
  ...
  // --- OFFICIAL EXAM DISTRIBUTION (Arrêté) ---

  const pick = (topicKey, type, count) => {
    return sample(
      fullBank.filter(q =>
        q.topicKey === topicKey &&
        (!type || q.type === type)
  ...
  questions = [
    ...
    ...pick("principes-valeurs", "knowledge", 5),
    ...pick("principes-valeurs", "situation", 6),

    // 2. Système institutionnel et politique
    ...pick("systeme-politique", null, 6),

    // 3. Droits et devoirs
    ...pick("droits-devoirs", "knowledge", 5),
    ...pick("droits-devoirs", "situation", 6),

    // 4. Histoire, géographie et culture
    ...pick("histoire-geographie-culture", null, 8),

    // 5. Vivre dans la société française
    ...pick("vivre-societe", null, 4)
  ];

  // Final shuffle so order looks natural
  shuffle(questions);
}
...
// 5. Initial wave: if nothing left → finish immediately
  ...
  const limit =
    (cfg.essentiel && cfg.essentiel.questionCount) ||
    options.limit ||
    12;

  const situations = fullBank.filter(q => q.type === "situation");

  questions = sample(
    situations,
    Math.min(limit, situations.length)
  );
...
// --- FIX: Log this attempt for detailed history review ---
  ...
  // ---------------------------------------------------------
// France Essentiel: show feedback after answer
// ---------------------------------------------------------
if (state.mode === "essentiel" && question.feedback) {
  const card = document.querySelector(".ce-card");
  if (card && !card.querySelector(".ce-feedback")) {
    const fb = document.createElement("div");
    fb.className = "ce-feedback";
    fb.textContent = question.feedback;
    card.appendChild(fb);
  }
}
...
if (state.mode === "topics") {
    ...
    // extract mistakes (use the real flag)
...
// --- RESET STATE FOR NEW WAVE ---
  ...
  state.allQuestions = wrong.slice();   // NEW: now the next wave uses only mistakes
  state.wave = (state.wave || 1) + 1; // FIX: Increment wave counter

  state.currentIndex = 0;

  // Reset counters (otherwise regression bar + repeated correct questions break)
  state.answered = 0;
  state.correct = 0;
  state.incorrect = 0;
...
function updateProgressBar() {
  ...
  async function computeTopicsRemaining() {
    ...
    // Primary: use explicit selected topics
    if (Array.isArray(state.selectedTopics) && state.selectedTopics.length) {
      selectedNorm = state.selectedTopics
        .map(k => topicLabels[k])
        .filter(Boolean)
        .map(lbl => normalizeLabel(lbl));
