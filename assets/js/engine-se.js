...
Engine.ensureBankLoaded = async function () {
  ...
  // Every SE question carries explanation {sv, en}. Shown after the answer
  // in every mode except the timed simulation (kept exam-like), and in the
  // post-test review of wrong answers.
  const EXPLAIN_TITLES = { sv: "Bra att veta", en: "Good to know" };
  ...
  function explanationText(rawQ) {
    ...
    const lang = getLang();
    return String(ex[lang] || ex.sv || ex.en || "").trim();
  }
