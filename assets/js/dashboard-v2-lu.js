(() => {
  ...
  // Luxembourg canonical topic = EN
  if (typeof topic === "object" && topic.en) {
    return topic.en;
  }
...
function buildTopicLabelMap(bank, lang) {
  ...
  bank.forEach(q => {
  if (!q.topic || typeof q.topic !== "object" || !q.topic.en) return;
  const key = q.topic.en;

  if (shortByLang && shortByLang[key]) {
    map[key] = shortByLang[key];
  } else {
    map[key] = q.topic[lang] || key;
  }
});
...
function computePerTopicProgressFromHistory(history) {
  ...
  // 1️⃣ INITIALIZE ALL TOPICS FROM BANK (this is what was missing)
  bank.forEach(q => {
  ...
  // 2️⃣ APPLY PROGRESS FROM HISTORY
  history.forEach((session) => {
    const questions = session.questions || [];

    questions.forEach((q) => {
      const topic = getTopicKey(q.mainTopic || q.topic);
      if (!perTopic[topic]) return;
...
topicsCard.innerHTML = `
  <div class="review-thankyou">
    <h3>${t("dashboard_review_thanks_title", "Thank you!")}</h3>
    <p>${t("dashboard_review_thanks_text", "Your feedback has been sent.")}</p>
  </div>
`;
