...
Engine.start = async function start(mode, options = {}) {
  ...
  const perTopic = {
    "Australia and its People": 5,
    "Beliefs, rights and liberties": 5,
    "Government and the law": 5,
    "Australian values": 5
  };
  ...
  Object.entries(perTopic).forEach(([topicLabel, count]) => {
    ...
    if (pool.length < count) {
      throw new Error(
        `Not enough questions for topic "${topicLabel}" (need ${count}, have ${pool.length})`
      );
    }
...
// ===================================================
// AUSTRALIAN CITIZENSHIP TEST – OFFICIAL RULES
// ===================================================
if (state.mode === "simulation") {

  // Rule 1: at least 75% overall (15 / 20)
  if (correct < 15) {
    passed = false;
  }

  // Rule 2: all 5 Australian values questions must be correct
  const valuesQuestions = state.questions.filter(q =>
    normalizeLabel(q.topicLabel) === normalizeLabel("Australian values")
  );
  ...
  if (valuesQuestions.length !== 5 || valuesCorrect !== 5) {
    passed = false;
  }
}
