// ============================================================
// DASHBOARD SEQUENTIAL PRACTICE — LUX
...
(function () {
  ...
  async function initDashboardSequential() {
    ...
    const COUNTRY = (window.CIVICEDGE_CONFIG && window.CIVICEDGE_CONFIG.country) || "lux";

    // Storage keys (namespaced)
    const KEY_INDEX = `civiclearn:${COUNTRY}:dashseq:index`;
    const KEY_LOG   = `civiclearn:${COUNTRY}:answer-log`;
    const KEY_STATS = `civiclearn:${COUNTRY}:progress`;     

    // Ensure bank base is available
// Hard bind LUX question bank (dashboard module must be autonomous)
    const bankBase = "/lux/banks/lux";

    let questions = [];
    try {
      const res = await fetch(`${bankBase}/sequential-lu.json`, { cache: "no-store" });
      questions = await res.json();
      if (!Array.isArray(questions)) throw new Error("sequential-lu.json invalid");
    } catch (err) {
      console.error("Failed to load sequential-lu.json:", err);
      card.style.display = "none";
      return;
    }
