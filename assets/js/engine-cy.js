/* CivicEdge Engine v6 – classic behavior preserved, multilingual bank support (Cyprus: el/en/ru) */
...
Engine.ensureBankLoaded = async function () {
  ...
  // ── "Καλό να γνωρίζετε" explanation card ──
  // Shown when a question carries explanation {el, en, ru} (every question in the CY bank). Shown after the answer
  // in every mode except the timed simulation (kept exam-like), and in the
  // post-test review of wrong answers.
  const EXPLAIN_TITLES = { el: "Καλό να γνωρίζετε", en: "Good to know", ru: "Полезно знать" };
  ...
  function explanationText(rawQ) {
    ...
    const lang = getLang();
    return String(ex[lang] || ex.el || ex.en || "").trim();
  }
  ...
  if (typeof rawQ.topic === "object") {
    return rawQ.topic[lang] || rawQ.topic.el || rawQ.topic.en || "";
  }
...
if (seenIdSet && seenIdSet.has(qid)) {
  ...
  function getLang() {
  return (state && state.lang) || window.CIVICEDGE_LANG || "el";
}
...
function getTopicDisplay(rawQ) {
  ...
  function getTextForLang(objOrStr) {
    ...
    if (typeof objOrStr === "object") {
      return objOrStr[lang] || objOrStr.el || objOrStr.en || "";
    }
  ...
  function getOptionsForLang(rawQ) {
    ...
    // New format: options: {el:[], en:[], ru:[]}
    if (typeof opts === "object" && !Array.isArray(opts)) {
      return opts[lang] || opts.el || opts.en || [];
    }
  ...
  let state = { lang: window.CIVICEDGE_LANG || "el" };
...
if (!answered.includes(question.id)) {
  ...
  writeJsonLS("civiclearn_answered_mcqs", answered);
    ...
    // === "Καλό να γνωρίζετε" EXPLANATION (all modes except the timed simulation) ===
