...
function computePerTopicProgressFromHistory(history) {
  ...
  history.forEach((session) => {
    ...
    questions.forEach((q) => {
      const topic = q.topicKey || q.topic || "Misc";
...
__globalChart = new Chart(ctx, {
    ...
    if (topicsCanvas) {
	  ...
	  const topicLabelsMap = window.CIVICEDGE_CONFIG?.topics?.topicLabels || {};
...
__topicsChart = new Chart(ctx, {
        type: "bar",
        data: {
          labels: topicKeys.map(
  k => topicLabelsMap[k] || k
), // ALL topics, not only those with progress
          datasets: [
            {
...
function maybeShowReviewCard({ mastery }) {
  const THRESHOLD = 0.85;
  const REVIEW_SHOWN_KEY = "cl_review_shown_v1";
  ...
  const topicsCanvas = document.getElementById("topicsChart");
  const topicsCard =
    topicsCanvas && topicsCanvas.closest
      ? topicsCanvas.closest(".db-metric-card")
      : null;
  if (!topicsCard) return;

  const originalHTML = topicsCard.innerHTML;
  ...
  const renderReview = () => {
    ...
    topicsCard.innerHTML = `
      <div class="review-card">
        <h3>${t("dashboard_review_title", "🎉 Great progress")}</h3>

        <p>
          ${t(
            "dashboard_review_intro",
            "You’ve completed over {pct}% of the training. A short comment can help future candidates prepare better."
          ).replace("{pct}", pct)}
        </p>
        ...
        <textarea
          id="reviewText"
          placeholder="${t(
            "dashboard_review_placeholder",
            "What helped you most in your preparation?"
          )}"
        ></textarea>

        <div class="review-actions">
          <button id="reviewSend" class="btn-primary">
            ${t("dashboard_review_send", "Send feedback")}
          </button>
          <button id="reviewCancel" class="btn-secondary">
            ${t("dashboard_review_cancel", "Not now")}
          </button>
        </div>
    ...
    const sendBtn = topicsCard.querySelector("#reviewSend");
    const cancelBtn = topicsCard.querySelector("#reviewCancel");
    const textEl = topicsCard.querySelector("#reviewText");
    const errorBox = topicsCard.querySelector("#reviewError");

    const restore = () => {
      topicsCard.innerHTML = originalHTML;

      // Re-init charts safely
      try {
        const perTopic = computePerTopicProgressFromProgressRaw(
          JSON.parse(localStorage.getItem("civicedge_progress")) || {}
        );
    ...
    sendBtn.onclick = async () => {
      ...
      if (!text) {
        errorBox.textContent = t(
          "dashboard_review_error_empty",
          "Please write a short comment before sending."
        );
  ...
  const res = await fetch(
    window.CIVICEDGE_CONFIG?.reviews?.submitUrl,
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        country: window.CIVICEDGE_CONFIG?.country || "unknown",
        product:
          window.CIVICEDGE_CONFIG?.examKey ||
          window.CIVICEDGE_CONFIG?.productKey ||
          "unknown",
        ...
        if (!res || !res.ok) throw new Error("submit_failed");
  ...
  if (
    ...
    window.CivicLearnI18n.onReady(renderReview);
  } else {
    renderReview();
  }
}
...
// Austria: only count relevant questions (same scope rules as engine)
const scopedBank = bank.filter(q =>
  q.scope === "federal" ||
  (q.scope === "bundesland" &&
   userBundesland &&
   q.bundesland === userBundesland)
);

const bankSize = scopedBank.length || 1;
...
// Austria: KPI must use the SAME scoped bank as donut
const mastery =
  scopedBank.length > 0
    ? masteredQuestions / scopedBank.length
    : 0;
...
safeText(document.getElementById("globalAnswered"), masteredQuestions + " " + label);
  ...
  const unit =
    streakDays === 1
      ? t("dashboard_streak_day_singular", "Tag")
      : t("dashboard_streak_day_plural", "Tage");
...
// Re-apply when i18n becomes ready (if applicable)
if (
  window.CivicLearnI18n &&
  typeof window.CivicLearnI18n.onReady === "function"
) {
...
function computePerTopicProgressFromProgressRaw(progressRaw) {
  ...
  bank.forEach(q => {
    // Apply Austria scope rules (same as engine)
if (
  q.scope === "bundesland" &&
  userBundesland &&
  q.bundesland !== userBundesland
...
const topic = q.topicKey || q.topic || "Misc";
  ...
  Object.entries(progressRaw || {}).forEach(([key, entry]) => {
	...
	// Apply Austria scope rules to progress entries
if (
  entry.topicKey === "bundesland" &&
  userBundesland &&
  entry.bundesland &&
  entry.bundesland !== userBundesland
) {
  return;
}

	const topic = entry.topicKey || entry.topic || "Misc";
