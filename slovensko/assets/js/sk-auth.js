/**
 * CivicAuth — Auth Guard (Slovensko)
 * ═══════════════════════════════════════════════════════════════
 * Include on every protected page.
 *
 * After loading, the global `slovenskoAuth` object is available:
 *   slovenskoAuth.email      — the logged-in user's email
 *   slovenskoAuth.userId     — the Supabase Auth user ID (UUID)
 *   slovenskoAuth.session    — the full session object
 *   slovenskoAuth.supabase   — the initialized Supabase client
 *   slovenskoAuth.signOut()  — logs out and redirects to login
 *   slovenskoAuth.ready      — Promise that resolves when auth check is complete
 */
...
(function () {
  ...
  const LOGIN_URL         = '/slovensko/login.html';
  ...
  auth.ready = (async () => {
    try {
      ...
      window.dispatchEvent(new CustomEvent('slovenskoAuthReady', { detail: auth }));
