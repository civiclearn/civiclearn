...
Engine.ensureBankLoaded = async function () {
  ...
  async function loadTrickySet() {
    ...
    try {
      const res = await fetch("/slovensko/data/tricky-sk.json");
  ...
  function isTricky(rawQ) {
    ...
    const sk = (qObj && typeof qObj === "object") ? (qObj.sk || qObj.en) : qObj;
    return sk && __trickySet.has(sk);
  }
...
if (!questions.length) {
  ...
  quizEl.innerHTML =
    `<div class="ce-card"><p>${t("status_no_data", "Žiadne údaje")}</p></div>`;
  return;
}
...
meta.textContent = t("question_x_of_y", "Otázka {x} z {y}")
...
// Official question badge (Slovak bank currently has no official=true questions,
// but we keep the code path + Slovak label in case we add a curated subset later)
if (q._raw?.official === true) {
  const officialBadge = createEl("div", "ce-q-official", "Oficiálna");
  header.appendChild(officialBadge);
}

// "Often incorrect" pill — shown for questions in the tricky-sk.json list
if (isTricky(q._raw)) {
  const trickyBadge = createEl("div", "ce-q-official ce-q-tricky", "Často chybné");
...
if (!(entry && entry.correct === 1)) {
  ...
  function renderTopicsRing(container, total, remaining) {
    ...
    const caption = createEl(
      ...
      t("topics_ring_caption", "{n} otázok zostáva vo vybraných témach")
  ...
  function finishQuiz(timeUp) {
    ...
    if (state.mode === "topics") {
      ...
      h2.textContent = t("topics_mastered_title", "Sada zvládnutá 🎉");

      launchConfetti();

      const sub = createEl("p", "muted");
      sub.setAttribute("data-i18n", "topics_mastered_sub");
      sub.textContent = t("topics_mastered_sub", "Správne ste odpovedali na všetky otázky v tejto sade.");

      const list = createEl("ul", "ce-result-list");
      const liTime = createEl("li");
      liTime.innerHTML = `<strong>${t("result_time", "Strávený čas")}:</strong> ${durationLabel}`;
  ...
  "button",
  "btn",
  t("topics_practice_again", "Precvičiť znovu")
);
...
btnBar.appendChild(practiceBtn);
      ...
      const continueBtn = createEl("button", "btn", t("topics_continue", "Pokračovať"));
      ...
      const backBtn = createEl("button", "btn secondary", t("topics_back_to_select", "Späť na témy"));
  ...
  .then(({ total, remaining }) => {
    ...
    h2.textContent = t("result_title", "Výsledky");

    const sub = createEl("p", "muted");
    sub.setAttribute("data-i18n", "result_subtitle");
    sub.textContent = t("result_subtitle", "Zhrnutie vašich výsledkov");
    ...
    liScore.innerHTML = `<strong>${t("result_score", "Skóre")}:</strong> ${percent}%`;

    const liCorrect = createEl("li");
    liCorrect.innerHTML = `<strong>${t("result_correct_answers", "Správne odpovede")}:</strong> ${correct}`;

    const liWrong = createEl("li");
    liWrong.innerHTML = `<strong>${t("result_wrong_answers", "Nesprávne odpovede")}:</strong> ${incorrect}`;
    ...
    if (state.mode === "traps") {
      ...
      trapsTitle.textContent = t("traps_fixed_title", "Opravené chyby");

      const trapsLine = createEl("p", "ce-result-traps-line");
      const tmpl = t("traps_fixed_line", "Opravili ste {fixed}. Zostáva {remaining}.");
    ...
    if (timeUp) {
      ...
      timeNote.textContent = t("test_time_up", "Čas vypršal!");
      card.appendChild(timeNote);
    }

    const btnBar = createEl("div", "ce-result-actions");

    const reviewBtn = createEl("button", "btn secondary", t("test_review_errors", "Zopakovať chyby"));
    reviewBtn.id = "reviewErrorsBtn";

    const restartBtn = createEl("button", "btn", t("test_restart", "Spustiť znovu"));
...
const key = `${topicLT}:${question.id}`;
  ...
  function startReviewMode() {
    ...
    if (!wrong.length) {
      quizEl.innerHTML = `
        <div class="ce-card"><p>${t("alert_no_errors", "Žiadne chyby")}</p></div>
      `;
  ...
  i18n.apply();
    ...
    let html = `
      <div class="ce-card" style="padding:24px;">
        <h2 style="margin-bottom:20px;">${t("review_title", "Prehľad chýb")}</h2>
    `;
...
<div class="ce-q-meta">${
  t("question_x_of_y", "Otázka {x} z {y}")
    .replace("{x}", i + 1)
    .replace("{y}", wrong.length)
}</div>

          <div class="ce-q-topic">${getTopicDisplay(q._raw) || q.topicLabel || ""}</div>
          ${q._raw?.official === true ? '<div class="ce-q-official">Oficiálna</div>' : ''}
