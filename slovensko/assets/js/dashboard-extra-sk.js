/* ─────────────────────────────────────────────────────────
   CivicLearn Dashboard Extra — Denmark-PR
...
(function () {
  ...
  function computeAndCacheGauge() {
    ...
    let badgeText;
    if (passed) badgeText = "Pripravený";
    else if (close) badgeText = "Takmer";
    else badgeText = "Treba cvičiť";
  ...
  function patchStatCards() {
    ...
    // Patch streak with Slovak 3-form pluralization
    ...
    if (tmStreak) {
      try {
        ...
        let word;
        if (streak === 1) word = "deň";
        else if (streak >= 2 && streak <= 4) word = "dni";
        else word = "dní";
        tmStreak.textContent = `${streak} ${word}`;
      } catch {}
    }
  }
