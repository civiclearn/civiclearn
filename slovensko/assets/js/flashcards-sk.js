/* ---------------------------------------------------------
   CivicLearn Flashcards — Slovensko (v2)
   --------------------------------------------------------- */
...
(function () {
  ...
  // ── Helpers ──
  function lang() { return window.CIVICEDGE_LANG || "sk"; }
  ...
  function renderChips() {
    ...
    if (seen.size === 0) {
      ...
      msg.className = "muted";
      msg.textContent = "Všetky témy sú zvládnuté";
  ...
  if (knownBtn) {
    knownBtn.addEventListener("click", () => {
      ...
      if (!filtered.length) {
        if (qText) qText.textContent = "";
        if (aText) aText.textContent = "";
        if (statusEl) statusEl.textContent = "Sada zvládnutá!";
        setPlaceholder(true);
        return;
      }
