/* ─────────────────────────────────────────────────────────
   CivicLearn Dashboard Extra — Luxembourg (Vivre ensemble)
   Vertical Bar Chart + Gauge + Celebration + Study time
   + Live updates from sequential widget
   Fork of dashboard-extra-se.js: fr/de/en texts, Lux defaults.
   ───────────────────────────────────────────────────────── */
...
(function () {
  ...
  // Gauge badge + chart tooltip texts (not in the i18n dictionary — kept inline like Sweden)
  const TXT = {
    fr: { pass: "Réussirait",  close: "Presque",     fail: "À travailler",     mastered: "% maîtrisé" },
    de: { pass: "Würde bestehen", close: "Fast geschafft", fail: "Noch üben",   mastered: "% gemeistert" },
    en: { pass: "Would pass",  close: "Almost there", fail: "Needs practice",   mastered: "% mastered" }
  };
  ...
  function computeAndCacheGauge() {
    ...
    const totalQ  = simCfg.questionCount || 40;
    const passScore = simCfg.passScore || 28;
    ...
    const badgeText = passed ? t("gauge_badge_pass", L.pass)
                    : close  ? t("gauge_badge_close", L.close)
                    :          t("gauge_badge_fail", L.fail);
      ...
      options: {
        ...
        plugins: {
          ...
          tooltip: {
            callbacks: {
              label: function (ctx) {
                const tpl = t("chart_tooltip_pct_mastered", "{n}" + L.mastered);
                return tpl.replace("{n}", ctx.parsed.y);
              }
            }
          }
        }
