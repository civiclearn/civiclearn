/**
 * CivicAuth — Auth Guard (Finland)
 * ═══════════════════════════════════════════════════════════════
 * Include on every protected page.
 *
 * After loading, the global `finlandAuth` object is available:
 *   finlandAuth.email      — the logged-in user's email
 *   finlandAuth.userId     — the Supabase Auth user ID (UUID)
 *   finlandAuth.session    — the full session object
 *   finlandAuth.supabase   — the initialized Supabase client
 *   finlandAuth.signOut()  — logs out and redirects to login
 *   finlandAuth.ready      — Promise that resolves when auth check is complete
 */
...
(function () {
  ...
  const LOGIN_URL         = '/finland/login.html';
  ...
  auth.ready = (async () => {
    try {
      ...
      window.dispatchEvent(new CustomEvent('finlandAuthReady', { detail: auth }));
